...
class LogProcessor(DataProcessor):
    def _is_valid_log(self, item: Any) -> bool:
        if not isinstance(item, dict):
            return False
        return all(isinstance(k, str) and isinstance(v, str) for k, v in item.items())
    ...
    def ingest(self, data: dict[str, str] | list[dict[str, str]]) -> None:
        ...
        if isinstance(data, list):
            ...
            items = [data]
        for item in items:
            converted = f"{items['log_level']}: {item['log_message']}"
...
def main() -> None:
    ...
    logg = [{'log_level': 'NOTICE', 'log_message': 'Connection to server'}, {'log_level': 'ERROR', 'log_message': 'Unauthorized access!!'}]
